...
void PushNodeVersion(CNode* pnode, CConnman* connman, int64_t nTime)
{
    ...
    if (fLogIPs) {
        ...
        LogPrint(BCLog::NET, "send version message: version %d, blocks=%d, us=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString().c_str(), nodeid);
    }
}
...
void Misbehaving(NodeId pnode, int howmuch)
{
    ...
    if (state->nMisbehavior >= banscore && state->nMisbehavior - howmuch < banscore) {
        LogPrintf("%s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED\n", __func__, state->name.c_str(), pnode, state->nMisbehavior - howmuch, state->nMisbehavior);
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock](CNode* pnode) {
        ...
        if (state.fPreferHeaderAndIDs && (!fWitnessEnabled || state.fWantsCmpctWitness) &&
            ...
            LogPrint(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", "PeerLogicValidation::NewPoWValidBlock",
                hashBlock.ToString().c_str(), pnode->GetId());
...
void static ProcessGetBlockData(CNode* pfrom, const Consensus::Params& consensusParams, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (send && !pfrom->fWhitelisted && ((((pfrom->GetLocalServices() & NODE_NETWORK_LIMITED) == NODE_NETWORK_LIMITED) && ((pfrom->GetLocalServices() & NODE_NETWORK) != NODE_NETWORK) && (chainActive.Tip()->nHeight - mi->second->nHeight > (int)NODE_NETWORK_LIMITED_MIN_BLOCKS + 2)))) {
...
bool static ProcessHeadersMessage(CNode* pfrom, CConnman* connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
...

    {
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            LogPrint(BCLog::NET, "more getheaders (%d) to end to peer=%d (startheight:%d)\n", pindexLast->nHeight, pfrom->GetId(), pfrom->nStartingHeight.load());
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                for (const CBlockIndex* pindex : reverse_iterate(vToFetch)) {
                    ...
                    LogPrint(BCLog::NET, "Requesting block %s from  peer=%d\n",
                        pindex->GetBlockHash().ToString().c_str(), pfrom->GetId());
                }
                if (vGetData.size() > 1) {
...
{
    LogPrint(BCLog::NET, "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand).c_str(), vRecv.size(), pfrom->GetId());
    ...
    if (strCommand == NetMsgType::REJECT) {
        ...
        LogPrint(BCLog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, peer=%d%s\n",
            cleanSubVer.c_str(), pfrom->nVersion.load(),
            pfrom->nStartingHeight.load(), addrMe.ToString().c_str(), pfrom->GetId(),
            remoteAddr.c_str());
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        if (!pfrom->fInbound) {
            ...
            LogPrintf("New outbound peer connected: version: %d, blocks=%d, peer=%d%s\n",
                pfrom->nVersion.load(), pfrom->nStartingHeight.load(), pfrom->GetId(),
                (fLogIPs ? strprintf(", peeraddr=%s", pfrom->addr.ToString()) : "").c_str());
        }
        ...
        if (nCMPCTBLOCKVersion == 1 || ((pfrom->GetLocalServices() & NODE_WITNESS) && nCMPCTBLOCKVersion == 2)) {
            ...
            if (State(pfrom->GetId())->fWantsCmpctWitness == (nCMPCTBLOCKVersion == 2))
        ...
        for (CInv& inv : vInv) {
            ...
            LogPrint(BCLog::NET, "got inv: %s  %s peer=%d\n", inv.ToString().c_str(), fAlreadyHave ? "have" : "new", pfrom->GetId());
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    LogPrint(BCLog::NET, "getheaders (%d) %s to peer=%d\n", pindexBestHeader->nHeight, inv.hash.ToString().c_str(), pfrom->GetId());
                }
            } else {
                pfrom->AddInventoryKnown(inv);
                if (fBlocksOnly) {
                    LogPrint(BCLog::NET, "transaction (%s) inv sent in violation of protocol peer=%d\n", inv.hash.ToString().c_str(), pfrom->GetId());
        ...
        if (vInv.size() > 0) {
            LogPrint(BCLog::NET, "received getdata for: %s peer=%d\n", vInv[0].ToString().c_str(), pfrom->GetId());
        }
        ...
        int nLimit = 500;
        LogPrint(BCLog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString().c_str(), nLimit, pfrom->GetId());
        ...
        LogPrint(BCLog::NET, "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString().c_str(), pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex)) {
        ...
        if (!AlreadyHave(inv) &&
            ...
            while (!vWorkQueue.empty()) {
                ...
                for (auto mi = itByPrev->second.begin();
                    ...
                    if (setMisbehaving.count(fromPeer))
                        continue;
                    if (AcceptToMemoryPool(mempool, stateDummy, porphanTx, &fMissingInputs2, &lRemovedTxn, false, 0)) {
            ...
            if (pfrom->fWhitelisted && gArgs.GetBoolArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
                ...
                if (!state.IsInvalid(nDoS) || nDoS == 0) {
                    ...
                    LogPrintf("Not relaying invalid transaction %s from whitelisted peer=%d (%s)\n", tx.GetHash().ToString().c_str(), pfrom->GetId(), FormatStateMessage(state).c_str());
                }
            }
        }
...

    {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrint(BCLog::NET, "received: feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString().c_str(), pfrom->GetId());
        }
    }
        ...
        LogPrint(BCLog::NET, "Unknown command \"%s\" from peer=%d\n", SanitizeString(strCommand).c_str(), pfrom->GetId());
    }
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(), CMessageHeader::MESSAGE_START_SIZE) != 0) {
        LogPrint(BCLog::NET, "PROCESSMESSAGE: INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()).c_str(), pfrom->GetId());
        pfrom->fDisconnect = true;
        return false;
    }
    ...
    if (!hdr.IsValid(chainparams.MessageStart())) {
        LogPrint(BCLog::NET, "PROCESSMESSAGE: ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetCommand()).c_str(), pfrom->GetId());
        return fMoreWork;
    }
    ...
    if (memcmp(hash.begin(), hdr.pchChecksum, CMessageHeader::CHECKSUM_SIZE) != 0) {
        LogPrint(BCLog::NET, "%s(%s, %u bytes): CHECKSUM ERROR expected %s was %s\n", __func__,
            SanitizeString(strCommand).c_str(), nMessageSize,
            HexStr(hash.begin(), hash.begin() + CMessageHeader::CHECKSUM_SIZE).c_str(),
            HexStr(hdr.pchChecksum, hdr.pchChecksum + CMessageHeader::CHECKSUM_SIZE).c_str());
        return fMoreWork;
    }
    ...
    try {
        ...
        if (strstr(e.what(), "end of data")) {
            LogPrint(BCLog::NET, "%s(%s, %u bytes): Exception '%s' caught, normally caused by a message being shorter than its stated length\n", __func__, SanitizeString(strCommand).c_str(), nMessageSize, e.what());
    ...
    if (!fRet) {
        LogPrint(BCLog::NET, "%s(%s, %u bytes) FAILED peer=%d\n", __func__, SanitizeString(strCommand).c_str(), nMessageSize, pfrom->GetId());
    }
    ...
    if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) && state.fSyncStarted) {
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= chainActive.Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                LogPrintf("Disconnecting outbound peer %d for old chain, best known block = %s\n", pto->GetId(), (state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>").c_str());
                pto->fDisconnect = true;
            } else {
                assert(state.m_chain_sync.m_work_header);
                LogPrint(BCLog::NET, "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n", pto->GetId(), (state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>").c_str(), state.m_chain_sync.m_work_header->GetBlockHash().ToString().c_str());
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                LogPrint(BCLog::NET, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, pto->GetId(), pto->nStartingHeight.load());
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    if (chainActive[pindex->nHeight] != pindex) {
                        LogPrint(BCLog::NET, "Announcing block %s not on main chain (tip=%s)\n",
                            hashToAnnounce.ToString().c_str(), chainActive.Tip()->GetBlockHash().ToString().c_str());
                    }

                    if (!PeerHasHeader(&state, pindex)) {
                        pto->PushInventory(CInv(MSG_BLOCK, hashToAnnounce));
                        LogPrint(BCLog::NET, "%s: sending inv peer=%d hash=%s\n", __func__,
                            pto->GetId(), hashToAnnounce.ToString().c_str());
                    }
                }
            }
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (nNow > state.nDownloadingSince + consensusParams.nPowTargetSpacing * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                LogPrintf("Timeout downloading block %s from peer=%d, disconnecting\n", queuedBlock.hash.ToString().c_str(), pto->GetId());
                pto->fDisconnect = true;
                return true;
            }
        }
