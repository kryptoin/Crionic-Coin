...

    {
        ...
        for (const CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s CRNC additional fees, %d delta bytes\n",
                ...
                hash.ToString().c_str(),
                FormatMoney(nModifiedFees - nConflictingFees).c_str(),
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("%s:  current best=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
        tip->GetBlockHash().ToString().c_str(), chainActive.Height(), log(tip->nChainWork.getdouble()) / log(2.0),
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", tip->GetBlockTime()).c_str());
    CheckForkWarningConditions();
}
...
void static UpdateTip(const CBlockIndex* pindexNew, const CChainParams& chainParams)
{
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
        pindexNew->GetBlockHash().ToString().c_str(), pindexNew->nHeight, pindexNew->nVersion,
        log(pindexNew->nChainWork.getdouble()) / log(2.0), (unsigned long)pindexNew->nChainTx,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexNew->GetBlockTime()).c_str(),
        GuessVerificationProgress(chainParams.TxData(), pindexNew), pcoinsTip->DynamicMemoryUsage() * (1.0 / (1 << 20)), pcoinsTip->GetCacheSize());
    if (!warningMessages.empty())
        LogPrintf(" warning='%s'", boost::algorithm::join(warningMessages, ", ").c_str());
    LogPrintf("\n");
}
...
static bool FindBlockPos(CDiskBlockPos& pos, unsigned int nAddSize, unsigned int nHeight, uint64_t nTime, bool fKnown = false)
{
    ...
    if ((int)nFile != nLastBlockFile) {
        if (!fKnown) {
            LogPrintf("Leaving block file %i: %s\n", nLastBlockFile, vinfoBlockFile[nLastBlockFile].ToString().c_str());
        }
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    LogPrintf("%s: last block file info: %s\n", __func__, vinfoBlockFile[nLastBlockFile].ToString().c_str());
...
bool CChainState::ReplayBlocks(const CChainParams& params, CCoinsView* view)
{
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) {
            ...
            LogPrintf("Rolling back %s (%i)\n", pindexOld->GetBlockHash().ToString().c_str(), pindexOld->nHeight);
    ...
    for (int nHeight = nForkHeight + 1; nHeight <= pindexNew->nHeight; ++nHeight) {
        ...
        LogPrintf("Rolling forward %s (%i)\n", pindex->GetBlockHash().ToString().c_str(), nHeight);
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus())) {
                            LogPrint(BCLog::REINDEX, "%s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash().ToString().c_str(),
                                head.ToString().c_str());
                            LOCK(cs_main);
