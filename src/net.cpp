    ...
    if (pszDest) {
        ...
        if (Lookup(pszDest, resolved, default_port, fNameLookup && !HaveNameProxy(), 256) && !resolved.empty()) {
            ...
            if (!addrConnect.IsValid()) {
                LogPrint(BCLog::NET, "Resolver returned invalid address %s for %s", addrConnect.ToString().c_str(), pszDest);
                return nullptr;
            }
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. %s is probably already running."), addrBind.ToString().c_str(), _(PACKAGE_NAME));
        else
        ...
        if (LookupHost(pszHostName, vaddr, 0, true)) {
            for (const CNetAddr& addr : vaddr) {
                if (AddLocal(addr, LOCAL_IF))
                    LogPrintf("%s: %s - %s\n", __func__, pszHostName, addr.ToString().c_str());
            }
        }
    }
#else
    ...
    if (getifaddrs(&myaddrs) == 0) {
        for (struct ifaddrs* ifa = myaddrs; ifa != nullptr; ifa = ifa->ifa_next) {
            ...
            if (ifa->ifa_addr->sa_family == AF_INET) {
                ...
                if (AddLocal(addr, LOCAL_IF))
                    LogPrintf("%s: IPv4 %s: %s\n", __func__, ifa->ifa_name, addr.ToString().c_str());
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    LogPrint(BCLog::NET, "sending %s (%d bytes) peer=%d\n", SanitizeString(msg.command.c_str()).c_str(), nMessageSize, pnode->GetId());
...
{
    return nNow + (int64_t)(log1p(GetRand(1ULL << 48) * -0.0000000000000035527136788) * average_interval_seconds * -1000000.0 + 0.5);
}
