...
QVariant HiveTableModel::data(const QModelIndex& index, int role) const
{
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Rewards:
            ...
            return BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->rewardsPaid) + " " + BitcoinUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + " (" + QString::number(rec->blocksFound) + " blocks mined)";
        }
