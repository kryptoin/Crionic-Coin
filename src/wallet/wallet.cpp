...
bool CWallet::MarkReplaced(const uint256& originalHash, const uint256& newHash)
{
    ...
    if (!walletdb.WriteTx(wtx)) {
        LogPrintf("%s: Updating walletdb tx %s failed", __func__, wtx.GetHash().ToString().c_str());
        success = false;
    }
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                while (range.first != range.second) {
                    if (range.first->second != tx.GetHash()) {
                        LogPrintf("Transaction %s (in block %s) conflicts with wallet transaction %s (both spend %s:%i)\n", tx.GetHash().ToString().c_str(), pIndex->GetBlockHash().ToString().c_str(), range.first->second.ToString().c_str(), range.first->first.hash.ToString().c_str(), range.first->first.n);
            ...
            if (depth >= 0 && output.n < it->second.tx->vout.size() &&
                ...
                if (ExtractDestination(FindNonChangeParentOutput(*it->second.tx, output.n).scriptPubKey, address)) {
                    result[address].emplace_back(
                        &it->second, output.n, depth, true, true, false);
                }
            }
        }
    }
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, const int nConfMine, const int nConfTheirs, const uint64_t nMaxAncestors, std::vector<COutput> vCoins, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet) const
{
    ...
    if (coinLowestLarger &&
        ...
        if (LogAcceptCategory(BCLog::SELECTCOINS)) {
            ...
            for (unsigned int i = 0; i < vValue.size(); i++) {
                if (vfBest[i]) {
                    LogPrint(BCLog::SELECTCOINS, "%s ", FormatMoney(vValue[i].txout.nValue).c_str());
                }
            }
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    LogPrintf("Fee Calculation: Fee:%d Bytes:%u Needed:%d Tgt:%d (requested %d) Reason:\"%s\" Decay %.5f: Estimation: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out) Fail: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out)\n",
        nFeeRet, nBytes, nFeeNeeded, feeCalc.returnedTarget, feeCalc.desiredTarget, StringForFeeReason(feeCalc.reason).c_str(), feeCalc.est.decay,
...
unsigned int CWallet::ComputeTimeSmart(const CWalletTx& wtx) const
{
    ...
    if (!wtx.hashUnset()) {
        if (mapBlockIndex.count(wtx.hashBlock)) {
            ...
            LogPrintf("%s: found %s in block %s not in index\n", __func__, wtx.GetHash().ToString().c_str(), wtx.hashBlock.ToString().c_str());
        }
    }
    return nTimeSmart;
}
