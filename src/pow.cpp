...
unsigned int GetNextHiveWorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params)
{
    ...
    if ((pindexLast->nHeight + 1 >= nSpeedFork) && (pindexLast->nHeight + 1 <= nSpeedFork + 25))
                ...
                for (const auto& tx : block.vtx) {
                    ...
                    if (tx->IsBCT(consensusParams, scriptPubKeyBCF, &beeFeePaid)) {
                        if (tx->vout.size() > 1 && tx->vout[1].scriptPubKey == scriptPubKeyCF) {
                            ...
                            CAmount expectedDonationAmount = (beeFeePaid + donationAmount) / consensusParams.communityContribFactor;
...
bool GetNetworkHiveInfo2(int& immatureBees, int& immatureBCTs, int& matureBees, int& matureBCTs, CAmount& potentialLifespanRewards, const Consensus::Params& consensusParams, bool recalcGraph)
{
    ...
    for (int i = forkHeight; i < tipHeight; i++) {
        ...
        if (consensusParams.isTestnet == true) {
            ...
            if (!(chainActive.Back24testnet(pindexPrev)->GetBlockHeader().IsHiveMined(consensusParams))) {
        ...
        if (consensusParams.isTestnet == false) {
            ...
            if (!(chainActive.Back24(pindexPrev)->GetBlockHeader().IsHiveMined(consensusParams))) {
        ...
        if (consensusParams.isTestnet == false) {
            ...
            if ((!(chainActive.Back(pindexPrev))->GetBlockHeader().IsHiveMined(consensusParams))) {
                    ...
                    for (const auto& tx : block.vtx) {
                        ...
                        if (tx->IsBCT(consensusParams, scriptPubKeyBCF, &beeFeePaidX)) {
                            if (tx->vout.size() > 1 && tx->vout[1].scriptPubKey == scriptPubKeyCF) {
                                ...
                                CAmount expectedDonationAmountX = (beeFeePaidX + donationAmountX) / consensusParams.communityContribFactor;
        ...
        if (consensusParams.isTestnet == true) {
            ...
            if ((!(chainActive.Backtestnet(pindexPrev))->GetBlockHeader().IsHiveMined(consensusParams))) {
...
bool GetNetworkHiveInfo3(int& immatureBees, int& immatureBCTs, int& matureBees, int& matureBCTs, CAmount& potentialLifespanRewards, const Consensus::Params& consensusParams, bool recalcGraph)
{
    ...
    CAmount potentialLifespanRewards2 = (consensusParams.beeLifespanBlocks * GetBlockSubsidy(pindexPrev->nHeight, consensusParams)) / consensusParams.hiveBlockSpacingTarget;
        ...
        for (int i = 67777; i < remTipHeight; i++) {
            ...
            if (consensusParams.isTestnet == false) {
                ...
                if (i >= (consensusParams.ratioForkBlock + totalBeeLifespan2)) {
                    ...
                    if ((!(chainActive.ReBack(pindexPrev))->GetBlockHeader().IsHiveMined(consensusParams))) {
            ...
            if (consensusParams.isTestnet == true) {
                ...
                if (i >= consensusParams.ratioForkBlock + totalBeeLifespan2) {
                    ...
                    if ((!(chainActive.ReBacktestnet(pindexPrev))->GetBlockHeader().IsHiveMined(consensusParams))) {
...
bool CheckHiveProof(const CBlock* pblock, const Consensus::Params& consensusParams)
{
    ...
    if (verbose)
        LogPrintf("CheckHiveProof: messageSig          = %s\n", HexStr(&messageSig[0], &messageSig[messageSig.size()]).c_str());
    ...
    if (verbose)
        LogPrintf("CheckHiveProof: honeyAddress        = %s\n", EncodeDestination(honeyDestination).c_str());
    ...
    CScript bctScriptPubKey;
    {
        ...
        if (communityContrib) {
            ...
            if (bct != nullptr) {
                if (bct->vout.size() < 2 || bct->vout[1].scriptPubKey != scriptPubKeyCF) {
                    LogPrintf("CheckHiveProof: Community contrib was indicated but not found\n");
                    return false;
                }
